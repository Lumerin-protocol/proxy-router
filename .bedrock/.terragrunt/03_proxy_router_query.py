...
cw_metric14 = os.environ["CW_METRIC14"]
cw_metric15 = os.environ["CW_METRIC15"]
cw_metric16 = os.environ["CW_METRIC16"]
cw_metric17 = os.environ["CW_METRIC17"]
cw_metric18 = os.environ["CW_METRIC18"]
cw_metric19 = os.environ["CW_METRIC19"]
cw_metric20 = os.environ["CW_METRIC20"]
cw_metric21 = os.environ["CW_METRIC21"]
cw_metric22 = os.environ["CW_METRIC22"]

########### GET MINER DATA FROM ProxyAPI ###########
...
def get_contractsv2_data():
    ...
    try:
        ...
        if contractsv2_data:  
            ...
            seller_hashrate_purchased = round((contractsv2_data["SellerTotal"]["RunningTargetGHS"] / 10**6), 4)
        ...
        try:
            with urllib.request.urlopen(base_req) as response:
                if response.getcode() == 200:
                    ...
                    # Check for API error using V2 response format
                    ...
                    # Success case - V2 API returns status '1' for success
                    if 'result' in data and data['result'].isdigit():
                        return data['result']
                    else:
                        print("Invalid result format in API V2 response")
                        return "0"
                else:
...
def lambda_handler(event, context):
    ...
    print(f"  -Purchased: {seller_hashrate_purchased}")
    print(f"  -Used: {hashrate_used}")
    print(f"  -Free: {hashrate_free}")
    print(f"\nMiners:")
    print(f"  -Total: {miners_total}")
    print(f"  -Vetting: {miners_vetting}")
