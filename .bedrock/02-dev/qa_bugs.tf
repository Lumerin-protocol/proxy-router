...
################################
locals {
  dns_name_bugs_use1_1_ext = "bugs."     #needs to inlude trailing "." for non prods
  dns_name_bugs_use1_1_int = "bugs-int." #needs to inlude trailing "." for non prods
  node_set_bugs_use1_1     = "a"
}

output "bugs_proxyrouter_remote" { value = var.special_nodes["qa_bugs_create"] ? "mssh -t ${module.bugs_use1_1[0].uninode_id[0]} -u ${var.account_shortname} titanadmin@${aws_route53_record.bugs_int_use1_1[0].name}" : "" }
output "bugs_proxyrouter_external" { value = var.special_nodes["qa_bugs_create"] ? "${aws_route53_record.bugs_ext_use1_1[0].name}:3333" : "" }

################################
# Proxyrouter NODE  
module "bugs_use1_1" {
  count              = var.special_nodes["qa_bugs_create"] ? 1 : 0
  ...
  node_protect       = false
  node_name_int      = "qa-bugs" #nodename root of instance(s)
  node_name_ext      = "qa-bugsext"
  ...
  node_qty_placement = ["${local.node_set_bugs_use1_1}"] # ["a", "b", "c", "b"] - module will count # of items and create in each az as specified 
...
resource "aws_route53_record" "bugs_ext_use1_1" {
  count    = var.special_nodes["qa_bugs_create"] ? 1 : 0
  ...
  name     = var.account_lifecycle == "prd" ? "${local.dns_name_bugs_use1_1_ext}${data.aws_route53_zone.public_lumerin_root.name}" : "${local.dns_name_bugs_use1_1_ext}${data.aws_route53_zone.public_lumerin.name}"
  type     = "A"
  ttl      = "300"
  records  = [module.bugs_use1_1[0].uninode_ext_ip[0]]
}

############# INTERNAL Route53 to node #############

# # ########## DNS Record local domain -internal
resource "aws_route53_record" "bugs_int_use1_1" {
  count    = var.special_nodes["qa_bugs_create"] ? 1 : 0
  provider = aws.special-dns
  zone_id  = var.account_lifecycle == "prd" ? data.aws_route53_zone.public_lumerin_root.zone_id : data.aws_route53_zone.public_lumerin.zone_id
  name     = var.account_lifecycle == "prd" ? "${local.dns_name_bugs_use1_1_int}${data.aws_route53_zone.public_lumerin_root.name}" : "${local.dns_name_bugs_use1_1_int}${data.aws_route53_zone.public_lumerin.name}"
  type     = "A"
  ttl      = "300"
  records  = [module.bugs_use1_1[0].uninode_ip[0]]
}