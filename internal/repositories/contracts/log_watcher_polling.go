...
func (w *LogWatcherPolling) Watch(ctx context.Context, contractAddr common.Address, mapper EventMapper, fromBlock *big.Int) (*lib.Subscription, error) {
	if fromBlock == nil {
		block, err := w.getBlockNumber(ctx)
		if err != nil {
			return nil, err
		}
		fromBlock = block
	...
	return lib.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			block, blockErr := w.getBlockNumber(ctx)
...
func (w *LogWatcherPolling) getBlockNumber(ctx context.Context) (*big.Int, error) {
