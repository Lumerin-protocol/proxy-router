...
import (
	...
	"gitlab.com/TitanInd/proxy/proxy-router-v3/internal/repositories/contracts/mocks"
)
...
func TestLogWatcherPolling(t *testing.T) {
	...
	ethClientMock := mocks.NewEthereumClientMock(t)
...
func TestWatchDoesntReturnEventsTwice(t *testing.T) {
	...
	_ = ethClientMock.EXPECT().HeaderByNumber(mock.Anything, mock.Anything).Return(&types.Header{Number: big.NewInt(2)}, nil)
