...
func TestMsgBusDataAddedToApiRepos(t *testing.T) {
	...
	if api.destRepo.DestJSONs[0].ID != string(dest.ID) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.nodeOperatorRepo.NodeOperatorJSONs[0].ID != string(nodeOperator.ID) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.contractRepo.ContractJSONs[0].ID != string(contract.ID) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.configRepo.ConfigInfoJSONs[0].ID != string(config.ID) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.minerRepo.MinerJSONs[0].ID != string(miner.ID) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.connectionRepo.ConnectionJSONs[0].ID != string(connection.ID) {
		t.Errorf("Failed to add connection to API repo after publish from msgbus")
	}
	fmt.Print("\nAPI Repos::\n")
	fmt.Println("Dest Repo: ", api.destRepo.DestJSONs)
	fmt.Println("NodeOperator Repo: ", api.nodeOperatorRepo.NodeOperatorJSONs)
	fmt.Println("Contract Repo: ", api.contractRepo.ContractJSONs)
	fmt.Println("Config Repo: ", api.configRepo.ConfigInfoJSONs)
	fmt.Println("Miner Repo: ", api.minerRepo.MinerJSONs)
	fmt.Println("Connection Repo: ", api.connectionRepo.ConnectionJSONs)

	fmt.Print("\n/// Update Msgbus Msgs //\n\n")
	ps.SetWait(msgbus.DestMsg, msgbus.IDString(dest.ID), dest)
	time.Sleep(time.Millisecond * 100)
	if api.destRepo.DestJSONs[0] != msgdata.ConvertDestMSGtoDestJSON(dest) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.nodeOperatorRepo.NodeOperatorJSONs[0].DefaultDest != msgdata.ConvertNodeOperatorMSGtoNodeOperatorJSON(nodeOperator).DefaultDest {
	...
	time.Sleep(time.Millisecond * 100)
	if api.contractRepo.ContractJSONs[0] != msgdata.ConvertContractMSGtoContractJSON(contract) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.configRepo.ConfigInfoJSONs[0] != msgdata.ConvertConfigInfoMSGtoConfigInfoJSON(config) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.minerRepo.MinerJSONs[0] != msgdata.ConvertMinerMSGtoMinerJSON(miner) {
	...
	time.Sleep(time.Millisecond * 100)
	if api.connectionRepo.ConnectionJSONs[0] != msgdata.ConvertConnectionMSGtoConnectionJSON(connection) {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.destRepo.DestJSONs) > 0 {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.nodeOperatorRepo.NodeOperatorJSONs) > 0 {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.contractRepo.ContractJSONs) > 0 {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.configRepo.ConfigInfoJSONs) > 0 {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.minerRepo.MinerJSONs) > 0 {
	...
	time.Sleep(time.Millisecond * 100)
	if len(api.connectionRepo.ConnectionJSONs) > 0 {
